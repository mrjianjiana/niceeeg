def epoching(args, data_part, seed):
	...
	for s in range(args.n_ses):
		...
		# Resample continuous raw data before epoching to reduce peak memory use.
		# Passing events lets MNE update event sample indices after resampling.
		if args.sfreq < sfreq:
			raw, events = raw.resample(args.sfreq, events=events)
			epoch_tmax = 1.0 - 1.0 / args.sfreq
		else:
			epoch_tmax = 1.0
		# * [0, 1.0] at target sampling rate; 250 Hz keeps 300 points before trimming baseline.
...
def mvnn(args, epoched_test, epoched_train):
	...
	for s in range(args.n_ses):
		...
		### Whiten the data ###
		whitened_test.append(np.asarray(np.reshape((np.reshape(session_data[0], (-1,
			session_data[0].shape[2],session_data[0].shape[3])).swapaxes(1, 2)
			@ sigma_inv).swapaxes(1, 2), session_data[0].shape), dtype=np.float32))
		whitened_train.append(np.asarray(np.reshape((np.reshape(session_data[1], (-1,
			session_data[1].shape[2],session_data[1].shape[3])).swapaxes(1, 2)
				@ sigma_inv).swapaxes(1, 2), session_data[1].shape), dtype=np.float32))
...
def save_prepr(args, whitened_test, whitened_train, img_conditions_train,
	...
	# Shuffle repetitions before inserting each condition. This avoids creating
	# a second full-size copy of merged_train with merged_train[:, idx].
	rep_idx = shuffle(np.arange(0, merged_train.shape[1]), random_state=seed)
	for i in range(len(np.unique(img_cond))):
		...
		merged_train[i] = ordered_data[rep_idx]
	# Insert the data into a dictionary
	train_dict = {
